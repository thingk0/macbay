...
# 判断不要な問題を自動修復: 記録済みの元パスが消えて失われたリンクを再作成し、
# 壊れたリンク・循環リンクを記録された外部コピーへ付け替えます
mb doctor --fix            # 実行前に確認プロンプト
mb doctor --fix --yes      # 確認をスキップ
mb doctor --fix --dry-run  # ファイルを変更せず修復内容をプレビュー
```

**`--fix` が修復する範囲**: マニフェスト記録が正で判断の余地がない場合のみです — `record_source_missing`（記録された元パスが消え外部コピーが残っている → シンボリックリンクを再作成）と `link_target_unavailable`/`link_circular`（リンクが使えず、実在する外部コピーの記録がある → リンクを付け替え）。リンクは常にボリュームの `MacBay/` レイアウト内のパスへだけ付け替えられ、修復に失敗してもデータは削除しません。判断が必要なファインディング（`local_data_detected`、`record_target_missing`、`link_record_mismatch`、ボリューム・マニフェストの問題、未完了の操作）は `Repairs` セクションでスキップとして報告され、修復後は再スキャンしてサマリーが最終状態を反映します。
