...
アプリケーションリンク、開発者キャッシュのリンク、接続中のボリュームに記録された MacBay の記録が実際の状態と一致しているかを確認します。`--fix` を付けない限り `doctor` は読み取り専用で、ファイルを変更しません:
...
# 判断不要な問題を自動修復: 壊れたリンク・循環リンクを
# 記録された外部コピーへ付け替えます
mb doctor --fix            # 修復対象を一覧表示してから確認プロンプト
mb doctor --fix --yes      # 確認をスキップ
mb doctor --fix --dry-run  # ファイルを変更せず修復内容をプレビュー
```

**`--fix` が修復する範囲**: `link_target_unavailable`/`link_circular` のうち、そのソースを主張するマニフェスト記録がちょうど1件で、外部コピーが実在する場合のみ — リンクをそのコピーへ付け替えます。リンクは常にボリュームの `MacBay/` レイアウト内のパスへだけ付け替えられ、置き換え直前にそのパスがまだリンクであることを再確認し、修復は `dock`/`repair` と同じボリューム単位ロックの下で実行され、失敗してもデータは削除しません。`record_source_missing` は意図的な削除の可能性があるためあえて自動修復しません。他の判断が必要なファインディング（`local_data_detected`、`record_target_missing`、`link_record_mismatch`、ボリューム・マニフェストの問題、未完了の操作）と同様に `Repairs` セクションでスキップとして報告します。修復後は再スキャンしてサマリーが最終状態を反映します。
...
> [!IMPORTANT]
> `doctor` は既定で読み取り専用です — `--fix` が唯一の書き込みモードで、シンボリックリンクの付け替えのみを行います。データの削除・移動は行わず、内蔵ディスクに別途記録を保存しないため、取り外した外部ボリュームは再接続するまで検証できません。
...
`mb undo` は、`mb history` の最新の成功エントリーが `dock` または `move` の場合にその操作を取り消します。dock は `mb undock`、move は `mb unmove` で元に戻します。まずそのコマンドを dry-run で実行するため、すでに undock 済みのアプリや MacBay のリンクではなくなったパスは何も変更する前に拒否され、`--yes` を付けない限り確認を求めます。

```sh
mb undo --dry-run   # 取り消す内容をプレビュー
mb undo             # 確認してから取り消す
mb undo --yes       # 確認をスキップ
```

対象は最新の成功エントリーだけです。失敗した試行は何も変更していないため読み飛ばし、より新しい操作を越えて古い操作を取り消すことはありません。それ以外の操作は理由とともに拒否します — 削除されたキャッシュ（`purge`、`xcode`）は復元できず、`cache --enable` と `teardown` は正確には元に戻せず、すでに取り消したエントリーは再度取り消しません。取り消し自体も `mb history` に記録されます。
