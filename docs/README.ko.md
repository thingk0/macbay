...
- **앱 복원 (`undock`)**: 연결된 볼륨의 MacBay 관리 앱을 내장 디스크로 안전하게 복원. 비관리 앱 및 확인 불가 항목은 상태 안내 및 관련 CLI 안내(`mb adopt`, `mb doctor`) 표시.
- **진단 (`doctor`)**: 링크 및 볼륨 레코드 검사 결과 탐색 및 문제별 권장 조치 확인.
- *(참고: 다중 선택, 검색, Xcode·캐시·adopt·repair 실행은 후속 버전에 추가될 예정이며 현재 버전에서는 CLI 명령으로 제공됩니다.)*
