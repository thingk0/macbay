...
# 자명한 문제 자동 복구: 끊어지거나 순환한 링크를
# 기록된 외장 사본으로 다시 연결
mb doctor --fix            # 복구 대상을 보여준 뒤 확인 프롬프트
mb doctor --fix --yes      # 확인 생략
mb doctor --fix --dry-run  # 변경 없이 복구 계획만 확인
```

**`--fix`가 복구하는 범위**: `link_target_unavailable`/`link_circular` 중 정확히 하나의 매니페스트 기록만 소스를 주장하고 그 외장 사본이 존재하는 경우뿐입니다 — 링크를 그 사본으로 재설정합니다. 링크는 항상 볼륨의 `MacBay/` 레이아웃 안쪽 경로로만 재연결되고, 교체 직전에 해당 경로가 여전히 링크인지 다시 확인하며, 복구는 `dock`/`repair`와 같은 볼륨별 잠금 아래에서 실행되고, 실패해도 데이터를 삭제하지 않습니다. `record_source_missing`은 의도적 삭제일 수 있어 의도적으로 자동 복구하지 않으며, 다른 판단 필요 파인딩(`local_data_detected`, `record_target_missing`, `link_record_mismatch`, 볼륨·매니페스트 문제, 미완료 작업)과 함께 `Repairs` 섹션에서 건너뛴 것으로 보고합니다. 복구 후에는 다시 검사해 요약이 최종 상태를 반영합니다.
...
> [!IMPORTANT]
> `doctor`는 기본적으로 읽기 전용입니다 — `--fix`가 유일한 쓰기 모드이며 심볼릭 링크 재설정만 수행합니다. 데이터를 삭제·이동하지 않으며 내부 디스크에 별도 기록을 저장하지 않으므로, 분리된 외장 볼륨은 다시 연결하기 전까지 검증할 수 없습니다.
