...
- **恢复应用 (`undock`)**：将已连接外置卷上的 MacBay 纳管应用安全恢复至内置 `/Applications`。非纳管应用及未确认链接显示对应状态与 CLI 指引（`mb adopt`, `mb doctor`）。
- **系统诊断 (`doctor`)**：按分类和状态浏览诊断发现，查看清晰高亮的针对性修复建议。
- *(注：多选、搜索以及 Xcode/缓存/adopt/repair 的 TUI 执行将在后续版本中支持。当前版本请使用对应 CLI 命令。)*
